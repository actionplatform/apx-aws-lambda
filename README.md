...
What the proxy makes per app: `ap-deploy-<org>-<project>-<app>` (CloudFormation, Lambda, API Gateway, logs on stacks named `ap-<org>-<project>-<app>*`, plus SAM's managed bucket) and `ap-exec-<org>-<project>-<app>` (the function's execution role, under the `ActionPlatformBoundary` policy the stack owns — edit it to cap what apps may do). The target passes the execution role as `ExecutionRoleArn` and refuses a `stack_name` outside the prefix. Admin calls need `org.manage` in the token; a deploy-job token has no scopes, so a compromised worker cannot widen a grant.
